...
/****** Object:  StoredProcedure [dbo].[topqueryreport]    Script Date: 03/13/2014 16:04:09 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @xmlo NVARCHAR(MAX) 
DECLARE @bodyo NVARCHAR(MAX)

SET @xmlo = CAST(( SELECT [startdate] AS 'td','',[enddate]  
        AS 'td'
FROM  #Tempo 
FOR XML PATH('tr'), ELEMENTS ) AS NVARCHAR(MAX)) 


SET @bodyo ='<html><body><H3>TEST TIME</H3> 
<table border = 1> 
<tr> 
<th> STARTDATE </th> <th> ENDDATE </th></tr>'    


SET @bodyo = @bodyo + @xmlo +'</table></body></html>'
...
( 
  [TSQL]  nvarchar(max), 
  [average_duration]  decimal(18,3), 
  [total_elapsed_time] decimal(18,3), 
  [exec_count]  int,
  dbname nvarchar(50) 
) 
INSERT INTO #Temp 
exec('
select top '+@totalnumber+' tsql, (cast((max(datediff(ms, ''01/01/2000 00:00:00.00'', cast(''01/01/2000 '' + total_elapsed_time as datetime))))/max(exec_count) as decimal))/1000 average_duration
, (cast(max(datediff(ms, ''01/01/2000 00:00:00.00'', cast(''01/01/2000 '' + total_elapsed_time as datetime))) as decimal)/1000) total_elapsed_time, max(exec_count) exec_count, max(dbname) dbname from [DBAMonitor].[dbo].[TOP_QUERY_BY_TIME]
...
( 
  [TSQL]  nvarchar(max), 
  [average_cpu_time]  decimal(18,3), 
  [total_cpu_time] decimal(18,3), 
  [exec_count]  int,
  dbname nvarchar(50) 
) 

INSERT INTO #Temp4 
exec('select top '+@totalnumber+' tsql, (cast((max(datediff(ms, ''01/01/2000 00:00:00.00'', cast(''01/01/2000 '' + total_cpu_time_ms as datetime))))/max(execution_count) as decimal))/1000 average_cpu_time
, (cast(max(datediff(ms, ''01/01/2000 00:00:00.00'', cast(''01/01/2000 '' + total_cpu_time_ms as datetime))) as decimal))/1000 total_cpu_time, max(execution_count) exec_count, max(dbname) dbname from [DBAMonitor].[dbo].[TOP_QUERY_BY_CPU]
