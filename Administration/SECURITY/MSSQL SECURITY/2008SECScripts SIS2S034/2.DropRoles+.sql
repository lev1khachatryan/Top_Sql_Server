...
AND name IN (SELECT [Database] FROM #Mappings2 WHERE [Server] = @@SERVERNAME AND IsActive = 1  AND ISNULL(@team,Team) = Team AND ISNULL(@db,[Database]) = [Database] AND [Database] NOT IN ('*')) -- specify the DBs for which you would like to run this script
ORDER BY name ;
