...
[IsActive]
FROM  #Mappings10
WHERE [Server] = @@SERVERNAME AND  ISNULL(@db,[Database]) = [Database] AND [Database] IN ('*')
    ...
    BEGIN
        ...
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                ...
                SET @sqlDB = 'USE ' + @dbname + ' ' + CHAR(13) + CHAR(10) 
                IF @FromWinGroup = 1 
                    BEGIN
                        SET @sql = @sqlDB
                            + 'IF EXISTS (SELECT * FROM sys.database_principals WHERE type = ''R'' AND name = '
                            + '''' + @Role + '''' + ')'
                            + 'INSERT  INTO #Temp EXEC sys.sp_helprolemember @rolename = '
                            + '''' + @Role + ''''
                        SET @sql = @sql
                            + ' IF   NOT  EXISTS (SELECT * FROM sys.database_principals WHERE type = ''G'' AND name = '
                            + '''' + @User + '''' + ')' + CHAR(13) + CHAR(10)
                            + ' OR NOT EXISTS (SELECT * FROM sys.database_principals WHERE type = ''R'' AND name = '
                            + '''' + @Role + '''' + ')' + ' OR '
                            + '( EXISTS (SELECT * FROM sys.database_principals WHERE type = ''G'' AND name = '
                            + '''' + @User + '''' + ')' + CHAR(13) + CHAR(10)
                            + ' AND  EXISTS (SELECT * FROM sys.database_principals WHERE type = ''R'' AND name = '
                            + '''' + @Role + '''' + ')' + 'AND ' + ''''
                            + @User + ''''
                            + ' NOT IN (SELECT  MemberName FROM #Temp) ) '
                            + CHAR(13) + CHAR(10)
                            
                 
                        SET @sql = @sql
                            + ' OR  NOT  EXISTS (SELECT * FROM master.dbo.syslogins WHERE loginname =  '
                            + '''' + @Login + '''' + ')'
                            + +CHAR(13) + CHAR(10)
                ...
                ELSE 
                    BEGIN
                        ...
                        SET @sql = @sql
                            ...
                            + ' OR NOT EXISTS (SELECT * FROM sys.database_principals WHERE type = ''R'' AND name = '
                            + '''' + @Role + '''' + ')' + ' OR '
                            + '( EXISTS (SELECT * FROM sys.database_principals WHERE type = ''S'' AND name = '
                            + '''' + @User + '''' + ')' + CHAR(13) + CHAR(10)
                            + ' AND  EXISTS (SELECT * FROM sys.database_principals WHERE type = ''R'' AND name = '
                            + '''' + @Role + '''' + ')' + 'AND ' + ''''
                            + @User + ''''
                ...
                SET @sql = @sql
                    + ' INSERT INTO  #Mappings18([Server],[Database],[Login],[FromWinGroup],[User],[Role])'
                    + CHAR(13) + ' VALUES( ' + '''' + @Server + '''' + ', '
                    + '''' + @dbname + '''' + ', ' + '''' + @Login + ''''
                    + ', ' + CAST(@FromWinGroup AS VARCHAR) + ', ' + ''''
                    + @User + '''' + ', ' + '''' + @Role + '''' + ')'
                    + CHAR(13) + 'DELETE FROM #Temp' + CHAR(13)
...
FOR SELECT [Database],[Role] FROM #Mappings18 WHERE[Server] = @@SERVERNAME 
...
WHILE @@FETCH_STATUS = 0 
    BEGIN
        ...
        PRINT ( @sql3 )
		INSERT INTO [DBAMonitor].[dbo].[SEC_SECURITYLOG] values (GETDATE(), @sql3)	
        EXEC(@sql3)
