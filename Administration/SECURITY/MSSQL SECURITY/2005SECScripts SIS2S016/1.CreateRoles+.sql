...
--Create sis_public, sis_dmladmin, sis_ddladmin and sis_admin roles on databases
...
AND name IN (SELECT DISTINCT [Database] FROM #Mappings1 WHERE [Server] = @@SERVERNAME 
AND ISNULL(@team,Team) = Team AND  ISNULL(@db,[Database]) = [Database] AND [Database] NOT IN ('*') ) -- specify the DBs for which you would like to run this script
ORDER BY name ;
...
WHILE @@FETCH_STATUS = 0 
    BEGIN
        ...
        FOR SELECT DISTINCT [Role] FROM #Mappings1 WHERE [Server] = @@SERVERNAME AND ISNULL(@team,Team) = Team
        OPEN Role_Cursor
        FETCH NEXT FROM  Role_Cursor INTO @Role
        WHILE @@FETCH_STATUS = 0 
            BEGIN
                PRINT 'Creating role ' + @Role + CHAR(13) + CHAR(10)
