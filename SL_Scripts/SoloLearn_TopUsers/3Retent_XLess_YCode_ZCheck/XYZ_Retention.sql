...
FROM   users AS u 
WHERE  u.registerdate BETWEEN '20180501' AND '20190201' -- 376397 
and CountryCode = 'us'
...
-- Retention for course completed users (beside html)
create table #CourseCompletedUsers
(
Userid int
)
insert into #CourseCompletedUsers
select distinct u.id
from UserCourses as uc
join Users as u on uc.UserId = u.Id
where CompletionDate is not null
...
where IsCompleted = 1 --CompletionDate is not null  
and courseid <> 1014
and u.CountryCode = 'us'
...
join 
(
	select u.id as UserID, u.RegisterDate
	from #CourseCompletedUsers as a
	join users as u on a.Userid = u.Id
) as sub on sub.UserID = uc.UserId
where uc.Date > sub.RegisterDate
and uc.Date <= DATEADD(week, 1, sub.RegisterDate)




select count(distinct userid)
from #CourseCompletedUsers
